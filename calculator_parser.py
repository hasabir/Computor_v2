    ...
    def parse(self, expression):
        try:
            ...
            if func_match and '?' not in expression:
                ...
                # Pass current variables to function so it can substitute predefined variables
                self.functions[func_name.lower()] = Function(func_name, var_name, func_expr, self.variables)
    ...
    def factor(self):
        try:
            ...
            if self.expression[self.position].isalpha() or self.expression[self.position] == '_':
                ...
                # Check for function call even if not explicitly defined (for variable substitution)
                # if (self.position < len(self.expression) and 
                #     self.expression[self.position] == '('):
                    # Parse function call with variable substitution
                    self.expression = self.expression[:start] + self.expression[self.position+1:]
                    self.position = start  # Reset position to start of variable name
                    arg_value = self.expr()
                    ...
                    # If it's a defined function, evaluate it
                    if var_name.lower() in self.functions:
                        func = self.functions[var_name.lower()]
                        ...
                        raise NameError(f"Function '{var_name}' is not defined")
